...
int fontChoice(){int64_t v=0;svc_config->get_int(mod_ctx,fontVar,&v);return int(std::clamp<int64_t>(v,0,9));}
...
ModResult initPresentation(){
 ...
 r=select("font","Font",{"consola","calamity-bold","lib-sans","lib-sans-bold","lib-serif","lib-serif-bold","press-start-2p","comic-sans","triforce","papyrus"},fontVar);if(r!=MOD_OK)return r;
